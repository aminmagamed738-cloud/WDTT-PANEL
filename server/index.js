 ...
 * - WebSocket tunnel
 * - VK call connector: auto-join with random name + captcha screenshot relay
 */
...
// Prevent Puppeteer/Chrome errors from crashing the panel server
process.on("uncaughtException",  err    => console.error("[WDTT] Exception:", err.message));
process.on("unhandledRejection", reason => console.error("[WDTT] Rejection:", String(reason)));
...
let bytesIn = 0;
let bytesOut = 0;
// For real bytes/s calculation
let _lastBytesIn = 0, _lastBytesOut = 0, _lastStatsTick = Date.now();
...
// ── VK call connector (Puppeteer) ─────────────────────────────────────────────

const RU_NAMES = [
  "Александр","Михаил","Дмитрий","Иван","Андрей",
  "Сергей","Артём","Максим","Кирилл","Никита",
  "Алексей","Евгений","Денис","Владимир","Павел",
];

let vkBrowser = null;
let vkPage    = null;
let vkState   = { active: false, url: null, name: null, status: "idle", error: null };
...
function vkBroadcast(payload) {
  const msg = JSON.stringify({ type: "vk-update", ...payload });
  wss_panel.clients.forEach(ws => { if (ws.readyState === 1) ws.send(msg); });
}

async function takeVkScreenshot() {
  if (!vkPage) return null;
  try {
    const buf = await vkPage.screenshot({ type: "jpeg", quality: 75 });
    return "data:image/jpeg;base64," + buf.toString("base64");
  } catch { return null; }
}

async function connectVkCall(url) {
  await disconnectVkCall();

  const executablePath = findChromium();
  if (!executablePath) throw new Error("Chromium не найден. Установите: apt install chromium");

  const randomName = RU_NAMES[Math.floor(Math.random() * RU_NAMES.length)]
    + "_" + Math.floor(1000 + Math.random() * 9000);

  vkState = { active: true, url, name: randomName, status: "connecting", error: null };
  vkBroadcast({ state: vkState, screenshot: null });

  addLog("info", `[VK] Подключение как "${randomName}" → ${url}`);
  ...
  vkBrowser = await puppeteer.launch({
    ...
    args: [
      ...
      "--disable-gpu", "--no-zygote", "--window-size=1280,720", "--lang=ru-RU",
    ],
  });
  ...
  try {
    ...
    addLog("warn", `[VK] Загрузка: ${e.message}`);
  }

  // Fill name field if present
  try {
    await vkPage.waitForSelector('input[type="text"]', { timeout: 5000 });
    await vkPage.evaluate((n) => {
      const el = document.querySelector('input[type="text"]');
      if (el) { el.focus(); el.select(); el.value = ""; }
    }, randomName);
    await vkPage.keyboard.type(randomName, { delay: 40 });
    addLog("info", `[VK] Имя введено: ${randomName}`);
  } catch { addLog("info", "[VK] Поле имени не найдено — продолжаем"); }

  // Click join button
  const clicked = await vkPage.evaluate(() => {
    const keywords = ["Войти в звонок","Войти","Присоединиться","Join","Подключиться","Начать"];
    for (const kw of keywords) {
      const btn = [...document.querySelectorAll("button")]
        .find(b => !b.disabled && b.textContent.trim().includes(kw));
      if (btn) { btn.click(); return kw; }
    }
    return null;
  });
  if (clicked) {
    addLog("info", `[VK] Нажата кнопка: "${clicked}"`);
    await new Promise(r => setTimeout(r, 2500));
  }

  vkState.status = "waiting_captcha";
  const screenshot = await takeVkScreenshot();
  vkBroadcast({ state: vkState, screenshot });
  addLog("success", "[VK] Готово. Если видите проверку бота — нажмите на неё в панели.");
}

async function disconnectVkCall() {
  if (vkBrowser) {
    await vkBrowser.close().catch(() => {});
    vkBrowser = null;
    vkPage = null;
  }
  vkState = { active: false, url: null, name: null, status: "idle", error: null };
  vkBroadcast({ state: vkState, screenshot: null });
}
  ...
  try {
    ...
    try {
      await fetch(`${u.protocol}//${u.hostname}`, {
        ...
        headers: { "User-Agent": "WDTT-Panel/4.1" },
      });
      clearTimeout(t);
      const ping = Date.now() - start;
      return { url, ping, status: ping < 300 ? "ok" : "slow", error: null };
    } catch (e) {
      clearTimeout(t);
      const msg = String(e);
      if (msg.includes("abort") || msg.includes("timeout"))
        return { url, ping: null, status: "unreachable", error: "Таймаут (>8s)" };
      return { url, ping: Date.now() - start, status: "slow", error: msg };
    }
  } catch (e) {
    return { url, ping: null, status: "unreachable", error: String(e) };
  }
}
...
app.get("/api/tunnel/xui-config", auth, (req, res) => {
  ...
  // Explicit VK + Russian white-list domains
  // geosite:geolocation-ru does NOT cover VK CDN domains — list them explicitly
  const vkDomains = [
    "domain:vk.com",
    "domain:vk.ru",
    "domain:vk.me",
  ...
  res.json({
    ...
    // Routing: Russian/VK traffic → WDTT SOCKS5
    routingRule: {
      type: "field",
      outboundTag: "wdtt-socks5",
      domain: vkDomains,
    },
    // Routing: only IPs (for IP-based connections)
    routingRuleIp: {
      type: "field",
      outboundTag: "wdtt-socks5",
      ip: ["geoip:ru"],
    ...
    testCommand: `curl --socks5 127.0.0.1:${socks5Port} https://vk.com`,
    testCommandHttp: `curl --proxytunnel -x http://127.0.0.1:${httpPort} https://vk.com`,
    vkDomains,
  });
});

// ── VK call connector routes ──────────────────────────────────────────────────

app.post("/api/vk/connect", auth, async (req, res) => {
  const { url } = req.body;
  if (!url) return res.status(400).json({ error: "Нет URL" });
  try { new URL(url); } catch { return res.status(400).json({ error: "Некорректный URL" }); }
  res.json({ connecting: true });
  connectVkCall(url).catch(e => {
    addLog("error", `[VK] Ошибка: ${e.message}`);
    vkState.status = "error";
    vkState.error  = e.message;
    vkBroadcast({ state: vkState, screenshot: null });
  });
});

app.post("/api/vk/disconnect", auth, async (req, res) => {
  await disconnectVkCall();
  addLog("info", "[VK] Отключились от звонка.");
  res.json({ disconnected: true });
});

// Click → forward to browser, take new screenshot, broadcast
app.post("/api/vk/click", auth, async (req, res) => {
  const { xPct, yPct } = req.body;
  if (!vkPage) return res.status(400).json({ error: "VK не подключён" });
...
app.get("/api/vk/screenshot", auth, async (req, res) => {
  const screenshot = await takeVkScreenshot();
  res.json({ screenshot, state: vkState });
});
...
wss_panel.on("connection", (ws) => {
  ...
  const tick = setInterval(() => {
    ...
    // Real bytes/s — calculated from actual proxy traffic
    const now = Date.now();
    const dt = (now - _lastStatsTick) / 1000;
    const rxBps = dt > 0 ? Math.round((bytesIn  - _lastBytesIn)  / dt) : 0;
    const txBps = dt > 0 ? Math.round((bytesOut - _lastBytesOut) / dt) : 0;
    ...
    tunnelState.bytesIn  = bytesIn;
    tunnelState.bytesOut = bytesOut;
    tunnelState.rxBps = rxBps;
    tunnelState.txBps = txBps;
