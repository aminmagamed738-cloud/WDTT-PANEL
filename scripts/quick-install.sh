...
# ── Chromium (для VK relay) ──────────────────────────────────────────────────────
step "Установка Chromium (для открытия VK звонков)"

CHROMIUM_FOUND=false
for bin in chromium chromium-browser google-chrome google-chrome-stable; do
  if command -v $bin &>/dev/null; then
    ok "Chromium найден: $(command -v $bin)"
    CHROMIUM_FOUND=true
    break
  fi
done
...
if [ "$CHROMIUM_FOUND" = false ]; then
  info "Устанавливаем Chromium..."
  case $OS_TYPE in
    debian)
      DEBIAN_FRONTEND=noninteractive apt-get install -y chromium >/dev/null 2>&1 || \
      DEBIAN_FRONTEND=noninteractive apt-get install -y chromium-browser >/dev/null 2>&1 || \
      warn "Не удалось установить Chromium. Установите вручную: apt install chromium"
      ;;
    rhel)
      $PKG_MANAGER install -y chromium >/dev/null 2>&1 || \
      warn "Не удалось установить Chromium: $PKG_MANAGER install chromium"
      ;;
    arch)
      pacman -Sy --noconfirm chromium >/dev/null 2>&1 || \
      warn "Не удалось установить Chromium: pacman -S chromium"
      ;;
    alpine)
      apk add --no-cache chromium >/dev/null 2>&1 || \
      warn "Не удалось установить Chromium: apk add chromium"
      ;;
    *)
      warn "Установите Chromium вручную для поддержки VK звонков"
      ;;
  esac
  for bin in chromium chromium-browser; do
    command -v $bin &>/dev/null && ok "Chromium установлен: $(command -v $bin)" && break
  done
fi
